  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: 'My Orders', value: stats.total, icon: Box, color: 'text-primary' },
          { label: 'Active', value: stats.active, icon: Activity, color: 'text-warning' },
          { label: 'Delivered', value: stats.completed, icon: ShieldCheck, color: 'text-success' },
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <button onClick={onRefresh} className="flex items-center gap-2 text-sm font-bold text-slate-400 hover:text-primary transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {filteredOrders.length > 0 ? (
            ...
            <div className="col-span-full py-20 text-center glass-card border-dashed border-white/10">
              <p className="text-slate-500">No matching orders in your history.</p>
            </div>
          )}
        </div>
      )}

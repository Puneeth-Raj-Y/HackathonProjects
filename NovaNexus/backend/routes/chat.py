...
def handle_chat(request: schemas.ChatRequest, db: Session = Depends(get_db)):
    try:
        ...
        logger.info(f"Successfully processed request. Replying with: {reply[:50]}...")
        return {
            "reply": reply,
            "intent": intent,
        ...
        # Fallback response so it doesn't crash the frontend
        return {
            "reply": f"⚠️ System encountered an internal error while processing: {str(e)}. Please try rephrasing your request.",
            "intent": "ERROR",
            "extracted_data": {}
        }
